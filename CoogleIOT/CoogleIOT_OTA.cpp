...
void CoogleIOT_OTA::initialize()
{
	...
	if(!configManager) {
		if(logger)
			logger->error(F("[OTA] Cannot initialize without a config manager"));
	...
	if(!configManager->loaded) {
		if(logger)
			logger->error(F("[OTA] Cannot initialize without a configuration (not loaded)"));
	...
	if(!sslClient && use_ssl) {
		...
		if(config->sec_allow_insecure_ssl) {
			if(logger)
				logger->warn(F("[OTA] Allowing Insecure SSL connections (via config)"));

			sslClient->setInsecure();
		}
	}

	if(!insecureWiFiClient && !use_ssl) {
...
bool CoogleIOT_OTA::loadAuthorities()
{
	...
	if(!configManager) {
		if(logger)
			logger->warn(F("[OTA] Config Manager Not Provided. Cannot Continue."));
	...
	if(!configManager->loaded) {
		...
		if(logger) 
			logger->warn(F("[OTA] Config Manager not loaded. Cannot Continue."));
	...
	if(!SPIFFS.exists("/certs.ar")) {
		...
		return !config->sec_allow_insecure_ssl;
	}
